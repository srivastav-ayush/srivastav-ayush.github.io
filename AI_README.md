...
| `writing/Articles.dc.html` | Writing/blog index — a two-level view (category hub → per-category list, open category in the URL hash). Data arrays `categoryMeta` (Travel / Tech / Education / Tools / Books) and `allArticles` (8 live essays, each linking to its own page in the same `writing/` folder). **Books is a different shape (Sep 2026, by request)**: it does NOT link out to essay pages. `allBooks` (in the same file) holds `{ key, title, author, rating (1-5), coverId, blurb (1 line), review (4-5 sentences) }`; the category view renders it as a flat list of expandable rows — cover thumbnail (2/3 ratio, `image-slot` id `book-cover-<key>`) + title + ★/☆ rating + author + one-line blurb, click/Enter/Space to expand the row and reveal the review paragraph in place. No separate pages, no dates, no read time — add a new book by appending one object to `allBooks`. |
