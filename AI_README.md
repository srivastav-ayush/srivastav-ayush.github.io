...
| `writing/Articles.dc.html` | Writing/blog index — a two-level view (category hub → per-category list, open category in the URL hash). Data arrays `categoryMeta` (Travel / Tech / Education / Tools) and `allArticles` (8 live essays, each linking to its own page in the same `writing/` folder). |
...
| `assets/img/writing/` | Writing-section photography — the 9 Kudremukh trek photos (≤1100px, 46–233 KB, re-encoded Aug 2026) and `timeplan-gantt.jpg`. |
