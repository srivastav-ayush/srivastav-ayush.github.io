...
| `reading/Reading.dc.html` / `reading/index.html` | Reading page — flat `allBooks` array (books, not essays), expandable rows, genre filter chips + sort/status menu. **Must stay byte-identical to `reading/index.html`** — see §8. Book covers are real files at `assets/img/books/<key>.jpg`, not `image-slot`s. Category taxonomy is documented at the end of §6 — categorize every new book against it. The Articles section (flat `articlesData` array, no covers) has its own Sort button (right-aligned on its own row below the section intro, `articleSortMode`/`articleSortMenuOpen` state) styled identically to the Books section's Sort button (same `t.borderStrong`/pill/44px/JetBrains Mono treatment) — Latest first / Oldest first only, no rating or status options since articles have neither. The row's left side is intentionally empty, reserved for a future category dropdown if the Articles list grows enough to need one. |
